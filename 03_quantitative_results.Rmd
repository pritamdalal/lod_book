...
In this section we consider the basic quantitative foundations of option pricing theory.  
...
Much of theoretical quantitative finance consists of theorems which are proved by using arbitrage arguements.  These theorems often involve question assumptions about the market; they are often of little to use to finance practitioners.
...
Properly defining the concept of a share of *stock* is a complicated legal exercise.  Similarly, a detailed account of how stocks are bought and sold in the secondary market would require its own large book.  The amount of detail you will ultimately know depends on how you intend to apply your knowledge.  For example, if you are an attorney that works on IPOs, you will need to know a lot about the legal stuff.  If you are a creating statistical arbitrage strategies, you will need be an expert on exchange microstructure.
...
A fairly recent innovation in the world of pooled investments is exchange traded funds (ETFs).  Relative to mutual funds and hedge funds, ETFs engage in simpler strategies that are easier to execute.  For this reason, it take fewer people to manage the fund and costs are low.  In addition, shares of ETF ownership are traded on exchanges in the exact same way as shares of stock.  ETF pay dividends in the same way that stocks do.  Here are some popular ETFs:
...
When you lend money, you are paid interest to compensate for the fact that you can't use the money to lent for other purposes. In this book we assume that loans are totally safe in that there is no risk of default.  
...
**Definition:** The time $t$ present value of a one dollar cash flow at time $T$ will be denoted by $D(t,T)$. I will call $D(t,T)$ a discount factor. I will call $D(t, T)$ a *discount factor*.
...
**Definition:** Suppose the current time is $t$ and consider a series of cash flows $C_{1}, \ldots, C_{n}$ occurring at future times $t_{1}, \ldots t_{n}$.  Then the time $t$ present value of this stream of cash flows is defined as:
...
Now, suppose for some particular $t^* \in (t, T)$ you want to work out an arrangement with your bank such that you will deposit one dollar at time $t^*$ and let it accumulate until $T$.  What is the accumulation factor that you should earn?  It turns out that this *forward accumulation factor* is determined by the spot yield curve.  We will denote the quantity $A(t, t^*, T)$. There is a simple no-arbitrage argument for this but I won't bother presenting it here because it's not import.  All you need to know is that it satisfies the following equation
\begin{align*}
...
We discussed vanilla (American and European) options at length in the previous section, but I will discuss them here for completeness of the chapter. 
...
Suppose that during the life of the options, that the underlying stock pays dividends $c_{1}, \ldots, c_{n}$ at times $t_{1}, \ldots, t_{n}$.
\begin{align*}
C(t, T, K) - P(t, T, K) &= D(t, T) \cdot (F(t, T) - K) \\
